...
class BacktestTests(unittest.TestCase):
    def test_loader_parses_historical_rows(self) -> None:
        ...
        self.assertEqual(points[0].funding_rate, Decimal("0.0005"))
    ...
    def test_backtest_replays_strategy(self) -> None:
        points = load_points(
            [
                ...
                {"ts": "2026-03-16T08:00:00+00:00", "price": "101", "funding_rate": "-0.0002", "liquidity_usd": "80000"},
            ]
        )
        result = FundingBacktester(fee_rate=Decimal("0.0001"), borrow_rate=Decimal("0.0001")).run(
            points,
            position_notional=Decimal("1000"),
        )
        self.assertEqual(result.total_return, Decimal("0.4"))
        self.assertEqual(len(result.equity_curve), 2)

    def test_report_is_consistent(self) -> None:
        points = load_points(
            [{"ts": "2026-03-16T00:00:00+00:00", "price": "100", "funding_rate": "0.0005", "liquidity_usd": "100000"}]
        )
        result = FundingBacktester().run(points, position_notional=Decimal("1000"))
        report = build_backtest_report(result)
        self.assertEqual(report["num_points"], 1)
        self.assertEqual(report["total_return"], "0.5000")
