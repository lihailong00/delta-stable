...
class PositionBookTests(unittest.TestCase):
    def test_net_exposure_and_balance_checks(self) -> None:
        ...
        self.assertEqual(book.net_exposure_by_symbol()["BTC/USDT"], Decimal("0.02"))
        self.assertFalse(book.is_balanced("BTC/USDT", tolerance=Decimal("0.01")))


class BalanceBookTests(unittest.TestCase):
    def test_available_margin_reflects_reserved_funds(self) -> None:
        ...
        self.assertEqual(balances.total_balance(exchange="binance"), Decimal("1000"))
        self.assertEqual(balances.available_margin("binance", "USDT"), Decimal("750"))


class CapitalAllocatorTests(unittest.TestCase):
    def test_allocator_applies_symbol_exchange_and_total_limits(self) -> None:
        allocator = CapitalAllocator(
            max_per_symbol=Decimal("500"),
            max_per_exchange=Decimal("800"),
        ...
        decision = allocator.allocate(
            ...
            current_symbol_notional=Decimal("200"),
            current_exchange_notional=Decimal("300"),
            current_total_notional=Decimal("700"),
        )
        self.assertEqual(decision.allocated_notional, Decimal("300"))
        self.assertEqual(decision.constrained_by, "symbol_limit")
