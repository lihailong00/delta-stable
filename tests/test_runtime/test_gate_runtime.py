...
class GateRuntimeTests(unittest.IsolatedAsyncioTestCase):
    async def test_public_ping_uses_live_http_transport(self) -> None:
        ...
        self.assertEqual(client.calls[0][1], "https://api.gateio.ws/api/v4/spot/currency_pairs")
    ...
    async def test_private_balance_check_uses_exchange_adapter(self) -> None:
        client = _HttpClient(
            [
                _Response(
                    [
                        {"currency": "USDT", "available": "100.5", "locked": "1.5"},
                        {"currency": "BTC", "available": "0.25", "locked": "0.0"},
                    ]
                )
            ]
        )
    ...
    async def test_ws_orderbook_stream_uses_public_subscription(self) -> None:
        ...
        self.assertEqual(ws.sent[0]["channel"], "spot.order_book")
    ...
    async def test_fetch_public_snapshot_uses_collector(self) -> None:
        ...
        self.assertEqual(snapshot["ticker"]["symbol"], "BTC/USDT")
        self.assertEqual(snapshot["orderbook"]["exchange"], "gate")
