...
class TestBinanceRuntime:

    async def test_public_ping_uses_live_http_transport(self) -> None:
        client = _HttpClient([_Response({})])
        runtime = BinanceRuntime.build(api_key='key', api_secret='secret', http_transport=HttpTransport(client=client), ws_connector=lambda endpoint: None)
        assert await runtime.public_ping()
        assert client.calls[0][1] == 'https://api.binance.com/api/v3/ping'

    async def test_private_balance_check_uses_exchange_adapter(self) -> None:
        client = _HttpClient([_Response({'balances': [{'asset': 'USDT', 'free': '100.0', 'locked': '1.0'}]})])
        runtime = BinanceRuntime.build(api_key='key', api_secret='secret', http_transport=HttpTransport(client=client), ws_connector=lambda endpoint: None)
        balances = await runtime.validate_private_access()
        assert balances['USDT'] == '101.0'

    async def test_ws_depth_subscription_streams_normalized_events(self) -> None:
        ws = _WebSocket([{'e': 'depthUpdate', 's': 'BTCUSDT', 'U': 157, 'u': 160, 'b': [['100.0', '1']], 'a': [['101.0', '2']]}])

        async def connector(endpoint: str):
            return ws
        http_client = _HttpClient([])
        runtime = BinanceRuntime.build(api_key='key', api_secret='secret', market_type=MarketType.SPOT, http_transport=HttpTransport(client=http_client), ws_connector=connector)
