...
class BaseWebSocketClientTests(unittest.TestCase):
    ...
    def test_heartbeat_flow(self) -> None:
        ...
        self.assertTrue(client.should_ping(now))
        ...
        self.assertFalse(client.should_reconnect(pong_time))

    def test_reconnect_when_pong_is_stale(self) -> None:
        client = _DummyWsClient()
        future = client.last_pong_at + timedelta(seconds=61)
        self.assertTrue(client.should_reconnect(future))
