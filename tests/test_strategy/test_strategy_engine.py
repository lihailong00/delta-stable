...
class SpotPerpStrategyTests(unittest.TestCase):
    def test_open_conditions_trigger_for_positive_funding(self) -> None:
        ...
        self.assertEqual(decision.action, StrategyAction.OPEN)
    ...
    def test_hedge_ratio_and_rebalance_trigger(self) -> None:
        ...
        decision = strategy.evaluate(
            SpotPerpInputs(
                ...
                perp_quantity=Decimal("0.95"),
            ),
            state=state,
        )
        self.assertEqual(strategy.target_hedge_ratio(spot_quantity=Decimal("1"), perp_quantity=Decimal("0.95")), Decimal("0.95"))
        self.assertEqual(decision.action, StrategyAction.REBALANCE)
    ...
    def test_close_when_holding_period_exceeded(self) -> None:
        ...
        self.assertEqual(decision.action, StrategyAction.CLOSE)


class PerpSpreadStrategyTests(unittest.TestCase):
    def test_cross_exchange_open_and_rebalance(self) -> None:
        ...
        open_decision = strategy.evaluate(
            PerpSpreadInputs(
                ...
                short_funding_rate=Decimal("0.0008"),
                long_price=Decimal("100"),
                short_price=Decimal("100"),
            )
        )
        self.assertEqual(open_decision.action, StrategyAction.OPEN)

        rebalance = strategy.evaluate(
            PerpSpreadInputs(
                symbol="ETH/USDT",
                long_exchange="okx",
                short_exchange="binance",
                ...
                short_quantity=Decimal("0.95"),
            ),
            state=StrategyState(is_open=True, hedge_ratio=Decimal("1")),
        )
        self.assertEqual(rebalance.action, StrategyAction.REBALANCE)
...
class StrategyEngineTests(unittest.TestCase):
    def test_state_machine_applies_open_rebalance_close(self) -> None:
        ...
        self.assertEqual(state.hedge_ratio, Decimal("0.98"))
