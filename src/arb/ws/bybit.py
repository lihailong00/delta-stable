...
from arb.ws.schemas import FillUpdatePayload, OpArgsMessage, OrderBookUpdatePayload, OrderUpdatePayload, PositionUpdatePayload, TickerUpdatePayload
...
class BybitWebSocketClient(BaseWebSocketClient):
    ...
    def build_subscribe_message(
        ...
        *,
        ...
        if self.private and channel in {"order", "execution", "position"}:
            return OpArgsMessage(op="subscribe", args=[channel])
        if symbol is None:
        ...
        return OpArgsMessage(op="subscribe", args=[topic])

    def build_auth_message(self, expires: int) -> OpArgsMessage:
    ...
    def build_ping_message(self) -> OpArgsMessage:
        return OpArgsMessage(op="ping", args=[])
    ...
    def _parse_orderbook(self, message: Mapping[str, object]) -> WsEvent:
        data = message["data"]
        return WsEvent(
            ...
            payload=OrderBookUpdatePayload(
                ...
                type=str(message["type"]) if message.get("type") is not None else None,
            ...
            events.append(
                WsEvent(
                    ...
                    payload=FillUpdatePayload(
                        ...
                        fee_asset=str(item["feeCurrency"]) if item.get("feeCurrency") is not None else None,
                    ),
                )
            )
        return events
